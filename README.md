...
### 🪜 Correlation Analysis: Spearman Rank Correlation in Churn Behaviors

Instead of relying on standard Pearson Correlation, we executed a **Spearman Rank Correlation** analysis during our exploratory phase to map out how behavioral metrics connect to customer churn.

#### Why Spearman is Mathematically Required for Telco Churn:
1. **Non-Linear, Monotonic Customer Lifecycles:** The relationship between a customer's tenure and their likelihood to churn is highly non-linear. Pearson fails to capture these curved relationships, whereas Spearman measures the consistent directional rank correlation.
2. **Extreme Skewness in Financial Fields:** `TotalCharges` is heavily right-skewed with natural outliers. Spearman operates on relative ranks rather than raw values.
3. **Robust Handling of Binary & Ordinal Targets:** Since `Churn` is binary and `NumServices` is discrete, Spearman provides a mathematically appropriate correlation measure.

```python
features_to_correlate=['tenure','MonthlyCharges','TotalCharges','NumServices','Churn']
spearman_matrix=df_processed[features_to_correlate].corr(method='spearman')
print(spearman_matrix['Churn'].sort_values(ascending=False))
```

**💡 Key Statistical Churn Signals:**
- Monthly Charges & Churn show a positive rank correlation.
- Tenure shows a strong negative rank correlation with Churn.
...
### 🌲 Interpretability: CatBoost Feature Importance & Business Drivers

To transition our classifier from a "black box" into a transparent decision-making tool, we extracted **Feature Importance** from the trained CatBoost model.
...
- **Tenure & Contract Type** are the strongest churn drivers.
- **Total Charges & Monthly Charges** are major financial anchors.
- **Tech Support & Online Security** bundles improve retention and reduce churn risk.
---
